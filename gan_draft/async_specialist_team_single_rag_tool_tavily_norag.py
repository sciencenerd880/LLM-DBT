...
# try loading from default .env if keys not found
if not GROQ_API_KEY:
...
class AsyncSpecialistTeamRAGTavily:
    """coordinates a multi-agent team for SharkTank pitch generation using an asynchronous workflow with Tavily search tools (no RAG)"""
    ...
    def __init__(self, output_dir, log_file_path, specialist_model_id="llama-3.3-70b-versatile", pitch_model_id="deepseek-r1-distill-llama-70b", max_tokens=4096, temperature=0.7, debug_mode=True):
        ...
        self.logger = logging.getLogger(f"async_team_tavily_norag_{os.path.basename(output_dir)}")
        ...
        with open(self.log_file_path, "a", encoding="utf-8") as f:
            f.write(f"=== ASYNC SPECIALIST TEAM WITH TAVILY (NO RAG) (Specialist Model: {self.specialist_model_id}, Pitch Model: {self.pitch_model_id}) ===\n\n")
        ...
        # Pitch Critic without RAG capabilities - using pitch model
        ...
        self.logger.info("Async Specialist Team with Tavily search tools initialized (NO RAG)")
    ...
    async def generate_pitch(self, facts, product_description, product_key):
        """generate a pitch for a SharkTank product using asynchronous specialists with Tavily search (no RAG)"""
        ...
        for name, result in zip(task_names, results):
            ...
            if "tool_calls" in result and result["tool_calls"]:
                self.logger.info(f"{name} had {len(result['tool_calls'])} successful tool calls")
        ...
        self.logger.info("Starting Pitch Critic (no RAG)...")
...
def setup_arg_parser():
    ...
    parser = argparse.ArgumentParser(description='Generate SharkTank pitches using an asynchronous multi-agent system with Tavily search tools (no RAG).')
...
async def main_async():
    """run the SharkTank pitch generator asynchronously with Tavily search tools (no RAG)"""
    parser = setup_arg_parser()
    args = parser.parse_args()
    
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    output_dir = f"./outputs/async_specialist_team_tavily_norag_{timestamp}"
    os.makedirs(output_dir, exist_ok=True)
    
    log_file_name = f"full_log_async_specialist_team_tavily_norag_{timestamp}.log"
    ...
    logger = logging.getLogger(f"async_pitch_generator_tavily_norag_{timestamp}")
    ...
    logger.info(f"Starting Async Specialist Team with Tavily (NO RAG), specialist model: {args.specialist_model}, pitch model: {args.pitch_model}, product key: {args.product_key}, max_tokens: {args.max_tokens}, temperature: {args.temperature}")
