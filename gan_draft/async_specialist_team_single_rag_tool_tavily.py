...
# try loading api keys from default .env if not found
if not GROQ_API_KEY:
...
if not TAVILY_API_KEY:
    raise ValueError(f"TAVILY_API_KEY not found in environment variables. Please set TAVILY_API_KEY in your environment or in the .env file.")

print(f"Loaded API keys from {env_path}")
print(f"OpenAI API key starts with: {OPENAI_API_KEY[:8]}...")

# force set openai api key in environment
...
def load_knowledge_base(
    ...
    if skip_embedding_check:
        ...
        try:
            # try different ways to count docs
...
class AsyncSpecialistTeamRAGTavily:
    """coordinates a multi-agent team for SharkTank pitch generation using an asynchronous workflow with both RAG and Tavily search tools"""
    
    def __init__(self, output_dir, log_file_path, specialist_model_id="llama-3.3-70b-versatile", pitch_model_id="deepseek-r1-distill-llama-70b", max_tokens=4096, temperature=0.7, debug_mode=True):
        """set up the team with output directory, logging, and model parameters"""
        self.logger = logging.getLogger(f"async_team_rag_tavily_{os.path.basename(output_dir)}")
        ...
        self.specialist_model_id = specialist_model_id
        self.pitch_model_id = pitch_model_id
        self.debug_mode = debug_mode
        self.logger.info(f"Using specialist model: {self.specialist_model_id}")
        self.logger.info(f"Using pitch model: {self.pitch_model_id}")
        self.logger.info(f"Debug mode: {'enabled' if self.debug_mode else 'disabled'}")
        
        self.log_file_path = log_file_path
        with open(self.log_file_path, "a", encoding="utf-8") as f:
            f.write(f"=== ASYNC SPECIALIST TEAM WITH RAG AND TAVILY (Specialist Model: {self.specialist_model_id}, Pitch Model: {self.pitch_model_id}) ===\n\n")
        ...
        def create_specialist_model():
            return Groq(
                id=self.specialist_model_id,
                api_key=GROQ_API_KEY,
                max_tokens=self.max_tokens,
                temperature=self.temperature
            )
        
        def create_pitch_model():
            return Groq(
                id=self.pitch_model_id,
                api_key=GROQ_API_KEY,
                max_tokens=self.max_tokens,
                temperature=self.temperature
            )
        ...
        # set up the team of specialist agents with Tavily search tool
        self.financial_strategist = Agent(
            name="Financial Strategist",
            model=create_specialist_model(),
        ...
        # Pitch Drafter - using pitch model, without RAG or search tools
        ...
        # Pitch Critic with RAG capabilities - using pitch model
        ...
        # Pitch Finalizer - using pitch model, without RAG or search tools
        ...
        self.logger.info("Async Specialist Team with both RAG and Tavily search tools initialized")
    ...
    async def run_agent_async(self, agent, agent_name, context):
        ...
        try:
            ...
            if response:
                ...
                # also check debug output for failed tool calls
                if debug_output:
                ...
                if len(tool_calls) > 0:
                    ...
                    # check if specialist made more than one search
                ...
                # use max count between tracked calls and debug output
    ...
    async def generate_pitch(self, facts, product_description, product_key):
        """generate a pitch for a SharkTank product using asynchronous specialists with both RAG and Tavily search"""
        ...
        # Specialist context with Tavily instructions
        ...
        # Track rate limiting across all agents
        tool_calls_count = 0
        ...
        # Create tasks for each specialist agent - NOTE: using specialist_context
        tasks = {
        ...
        # track progress for each specialist
        ...
        # process results from each specialist
        ...
        # prepare context for pitch drafter using base context without tool instructions
        ...
        with open(os.path.join(self.output_dir, "metrics.json"), "w", encoding="utf-8") as f:
            json.dump({
                ...
                "models_used": {
                    "specialist_model": self.specialist_model_id,
                    "pitch_model": self.pitch_model_id
                },
                "agent_input_lengths": {
                ...
                "tool_usage": {
                    "tavily_enabled_for_specialists": True,
                    "tavily_tool_calls_count": tool_calls_count,
...
def setup_arg_parser():
    ...
    parser = argparse.ArgumentParser(description='Generate SharkTank pitches using an asynchronous multi-agent system with both RAG and Tavily search tools.')
...
async def main_async():
    """run the SharkTank pitch generator asynchronously with both RAG and Tavily search tools"""
    parser = setup_arg_parser()
    args = parser.parse_args()
    
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    output_dir = f"./outputs/async_specialist_team_rag_tavily_{timestamp}"
    os.makedirs(output_dir, exist_ok=True)
    
    log_file_name = f"full_log_async_specialist_team_rag_tavily_{timestamp}.log"
    ...
    logger = logging.getLogger(f"async_pitch_generator_rag_tavily_{timestamp}")
    ...
    logger.info(f"Starting Async Specialist Team with RAG and Tavily, specialist model: {args.specialist_model}, pitch model: {args.pitch_model}, product key: {args.product_key}, max_tokens: {args.max_tokens}, temperature: {args.temperature}")
