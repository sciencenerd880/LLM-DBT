...
class AsyncSpecialistTeamDDG:
    """coordinates a multi-agent team for SharkTank pitch generation using an asynchronous workflow with DuckDuckGo search tools"""
    ...
    def __init__(self, output_dir, log_file_path, model_id="llama-3.3-70b-versatile", max_tokens=4096, temperature=0.7, debug_mode=True):
        ...
        self.logger = logging.getLogger(f"async_team_ddg_{os.path.basename(output_dir)}")
        ...
        with open(self.log_file_path, "a", encoding="utf-8") as f:
            f.write(f"=== ASYNC SPECIALIST TEAM WITH DUCKDUCKGO (Model: {self.model_id}) ===\n\n")
        
        self.max_tokens = max_tokens
        self.temperature = temperature
        self.logger.info(f"Using max_tokens: {self.max_tokens}, temperature: {self.temperature}")
        
        # Initialize DuckDuckGo search tool - simpler initialization
        self.ddg_tools = DuckDuckGoTools(fixed_max_results=3)  # Limit results to reduce rate limiting
        ...
        # Tool instructions for all specialist agents - following pattern from Google implementation
        ...
        self.logger.info("Async Specialist Team with DuckDuckGo search tools initialized")
    ...
    async def run_agent_async(self, agent, agent_name, context):
        ...
        try:
            ...
            if response:
                ...
                # track both successful and rate-limited tool calls
                tool_calls = []
                tool_attempts = 0
                
                # check standard locations for tool_calls
                ...
                # look for failed tool calls in debug output
                if debug_output:
                    # count all tool call attempts
                ...
                if len(tool_calls) > 0:
                    ...
                    # warn if multiple tool calls were made
                    if len(tool_calls) > 1:
                ...
                # look for mentions of tool usage in content
                mentions = 0
                if "duckduckgo" in content_for_conversation.lower() or "search tool" in content_for_conversation.lower():
                    # count references to search tools
                    mentions = (
                        content_for_conversation.lower().count("duckduckgo") + 
                    ...
                    if mentions > 0 and tool_attempts == 0:
                        ...
                        tool_attempts = max(1, mentions // 2)  # estimate tool attempts from mentions
                
                # use highest count from all sources
    ...
    async def generate_pitch(self, facts, product_description, product_key):
        """generate a pitch for a SharkTank product using asynchronous specialists with DuckDuckGo search"""
        ...
        # run tasks in parallel using task tracking dict
        ...
        for name, result in zip(task_names, results):
            ...
            # check for rate limiting in debug output
            if "response" in result and hasattr(result["response"], "debug_output"):
                ...
                if "202 Ratelimit" in debug_output:
                    rate_limit_encountered = True
            
            if debug_output := getattr(result.get("response", {}), "_debug_output", None):
                all_debug_outputs += debug_output
                if "202 Ratelimit" in debug_output:
                    rate_limit_encountered = True
        ...
        # prep context for pitch drafter without tool instructions
...
def setup_arg_parser():
    ...
    parser = argparse.ArgumentParser(description='Generate SharkTank pitches using an asynchronous multi-agent system with DuckDuckGo search tools.')
...
async def main_async():
    """run the SharkTank pitch generator asynchronously with DuckDuckGo search tools"""
    parser = setup_arg_parser()
    args = parser.parse_args()
    
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    output_dir = f"./outputs/async_specialist_team_ddg_{timestamp}"
    os.makedirs(output_dir, exist_ok=True)
    
    log_file_name = f"full_log_async_specialist_team_ddg_{timestamp}.log"
    ...
    logger = logging.getLogger(f"async_pitch_generator_ddg_{timestamp}")
    ...
    logger.info(f"Starting Async Specialist Team with DuckDuckGo search tools, model: {args.model}, product key: {args.product_key}, max_tokens: {args.max_tokens}, temperature: {args.temperature}")
