...
class AsyncSpecialistTeam:
    """coordinates a multi-agent team for SharkTank pitch generation using an asynchronous workflow"""
    ...
    def __init__(self, output_dir, log_file_path, model_id="llama-3.3-70b-versatile", max_tokens=4096, temperature=0.7, debug_mode=True):
        ...
        self.logger = logging.getLogger(f"async_team_{os.path.basename(output_dir)}")
        ...
        with open(self.log_file_path, "a", encoding="utf-8") as f:
            f.write(f"=== ASYNC SPECIALIST TEAM (Model: {self.model_id}) ===\n\n")
        ...
        self.logger.info("Async Specialist Team initialized")
    ...
    async def generate_pitch(self, facts, product_description, product_key):
        ...
        # prep context for drafter with specialist suggestions
...
def setup_arg_parser():
    ...
    parser = argparse.ArgumentParser(description='Generate SharkTank pitches using an asynchronous multi-agent system.')
...
async def main_async():
    """run the SharkTank pitch generator asynchronously"""
    parser = setup_arg_parser()
    args = parser.parse_args()
    
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    output_dir = f"./outputs/async_specialist_team_{timestamp}"
    os.makedirs(output_dir, exist_ok=True)
    
    log_file_name = f"full_log_async_specialist_team_{timestamp}.log"
    ...
    logger = logging.getLogger(f"async_pitch_generator_{timestamp}")
    ...
    logger.info(f"Starting Async Specialist Team with model: {args.model}, product key: {args.product_key}, max_tokens: {args.max_tokens}, temperature: {args.temperature}")
