...
from async_specialist_team_single_tool_tavily import AsyncSpecialistTeamTavily, load_data, TeeLogger, strip_ansi_codes, strip_thinking_tokens

def setup_arg_parser():
    """set up and return the argument parser"""
    parser = argparse.ArgumentParser(description='Generate SharkTank pitches for all instances using an asynchronous specialist team with Tavily search tools.')
...
async def process_instance_async(instance_key, instance_data, base_output_dir, args, logger):
    ...
    try:
        ...
        result_stats = {
            ...
            "Framework": "async_specialist_team_tavily",
...
def save_to_csv(results, output_dir, logger):
    ...
    csv_file = os.path.join(output_dir, f"async_specialist_team_tavily_all_results_compile_{timestamp}.csv")
...
async def main_async():
    """main async function to run pitch generation for all instances with Tavily search tools"""
    parser = setup_arg_parser()
    args = parser.parse_args()
    
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    base_output_dir = f"./evaluation_outputs/async_specialist_team_tavily_all_{timestamp}"
    ...
    logger = logging.getLogger("async_pitch_tavily_all_instances")
    ...
    logger.info(f"Starting SharkTank pitch generator for all instances using async specialist team with Tavily search tools")
    ...
    print(f"All done! Processed {len(results)} instances with Tavily search tools.")
    print(f"Results saved to: {base_output_dir}")
    print(f"CSV file: {os.path.join(base_output_dir, f'async_specialist_team_tavily_all_results_compile_{csv_timestamp}.csv')}")
